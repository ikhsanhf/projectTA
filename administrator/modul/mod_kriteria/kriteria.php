...
<script language="JavaScript" type="text/JavaScript">
 ...
 while ($data = mysql_fetch_array($hasil))
 {
   ...
   // membuat option klasifikasi untuk masing-masing kelas
  ...
  default:
    if ($_SESSION[leveluser]=='admin' || $_SESSION['leveluser']=='pendidikan'){
	 ...
	 <div class="box box-body box-solid">
				...
				<div class="box-body">
					<?php echo"<a  class ='btn  btn-success btn-flat' href='?module=kriteria&act=tambahkriteria'>ADD </a>"; ?>
					<br><br><br>
							...
							$no=1;
							  while ($r=mysql_fetch_array($tampil_kriteria)){
								...
								<td>$r[kriteria]</td>
								 ...
								 <td><a href='?module=kriteria&act=editkriteria&id=$r[id]' title='Edit' class='btn btn-primary btn-xs'>Edit</a> |
									 <a href=javascript:confirmdelete('$aksi?module=kriteria&act=hapus&id=$r[id]') title='Delete'  class='btn btn-danger btn-xs'>Delete</a> | <a href='?module=kriteria&act=listhimpunankriteria&id=$r[id]' title='input Data Kriteria' class='btn btn-primary btn-xs'>Input Data Kriteria</a></td></tr>";
								$no++;
								}
						echo "</tbody></table>";
					?>
    ...
    if ($_SESSION[leveluser]=='admin' || $_SESSION['leveluser']=='pendidikan'){
         echo "
		  ...
		  <div class='box box-body box-solid'>
				...
				<div class='box-body'>
						<form method=POST action='$aksi?module=kriteria&act=input_kriteria' enctype='multipart/form-data' class='form-horizontal'>
    ...
    if ($_SESSION[leveluser]=='admin' || $_SESSION['leveluser']=='pendidikan'){
        ...
        echo "
		  ...
		  <div class='box box-body box-solid'>
				...
				<div class='box-body'>
						<form method=POST action='$aksi?module=kriteria&act=update_kriteria'  class='form-horizontal'>
							...
							$no=1;
							  while ($r=mysql_fetch_array($tampil_kriteria)){
								...
								<td>$r[kriteria]</td>
								 ...
								 <td><a href='?module=kriteria&act=listhimpunankriteria&id=$r[id]' title='input Data Kriteria' class='btn btn-primary btn-xs'>Input Data Kriteria</a> 
									</td></tr>";
								$no++;
								}
						echo "</tbody></table>";
...
//bagian himpunan kriteria	
	...
	?>
	 ...
	 <div class="box box-body box-solid">
				...
				<div class="box-body">
					...
					<?php echo "<a  class ='btn  btn-success btn-flat' href='?module=kriteria&act=tambahhimpunan&id=$a[id]'>ADD </a>"; ?>
					<br><br><br>
						...
						<?php 
							$no=1;
							  while ($r=mysql_fetch_array($tampil_himpunankriteria)){
								...
								<td><a href='?module=kriteria&act=edithimpunankriteria&id=$r[id_hk]' title='Edit' class='btn btn-primary btn-xs'>Edit</a> 
									 <a href='$aksi?module=kriteria&act=hapus_himpunan&id=$r[id_hk]&id_kriteria=$r[id_kriteria]' title='Delete'  class='btn btn-danger btn-xs'>Delete</a></td></tr>";
								$no++;
								}
						echo "</tbody></table>";
					?>
	...
	echo "
		  ...
		  <div class='box box-body box-solid'>
				...
				<div class='box-body'>
						<form method=POST action='$aksi?module=kriteria&act=input_himpunan' class='form-horizontal'>
	...
	echo "
		  ...
		  <div class='box box-body box-solid'>
				...
				<div class='box-body'>
						<form method=POST action='$aksi?module=kriteria&act=update_himpunan' class='form-horizontal'>
							<input type=hidden name='id_kriteria' class='form-control' Placeholder='Masukan Data' value='$a[id]'>							 
...
<div class="box box-body box-solid">
				...
				<div class="box-body">
                                   <form method="POST" action="modul/mod_kriteria/reset.php">
									...
									echo "<tr class='warnabaris' >
											...
											<td>$r[nama_lengkap]</td>";
												...
												while($k=mysql_fetch_array($kelas)){
														...
														echo"<td><a href=?module=kelas&act=detailkelas&id=$k[id_kelas] title='Detail Kelas'>$k[nama]</a></td>";
												}
										echo "<td><a href=?module=kriteria&act=editklasifikasi&id=$r[id_siswa] class='btn btn-primary btn-xs'>Edit Klasifikasi</a> 
											 ";
                                                $cek = mysql_query("SELECT * FROM tbl_klasifikasi WHERE id_siswa='$r[id_siswa]'");
                                                if (mysql_num_rows($cek)>0) echo "Data Sudah terisi"; else echo "Data Belum terisi";
							...
							<ul class='nav nav-tabs'>
								...
								<div class='tab-content'>
									<div class='active tab-pane' id='activity'>
										<div class='post'>
										<form method=POST action='$aksi?module=kriteria&act=input_klasifikasi' ' class='form-horizontal'>
											...
											while ($f = mysql_fetch_array($kriteria)){
												...
												<div class='form-group'>
													...
													<select name='id_hk$i' class=' form-control  '  >
														 ...
														 echo "<option ";
														 			$cek = mysql_num_rows(mysql_query("SELECT * FROM tbl_klasifikasi WHERE id_siswa='$siswa[id_siswa]' and id_hk='$r[id_hk]'"));
														 


														 if($cek > 0) echo ' selected ';
														 echo"value=$r[id_hk]>$r[nama]</option>";
